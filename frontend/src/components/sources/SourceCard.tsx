...
function getNotebookColorClass(notebookName: string) {
  let hash = 0;
  for (let i = 0; i < notebookName.length; i++) {
    hash = notebookName.charCodeAt(i) + ((hash << 5) - hash);
  }
  const colors = [
        ...
        <div className="flex items-start justify-between gap-3 mb-1">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* Origin Notebook badge (for aggregated notebooks) */}
              {source.origin_notebook_name && source.origin_notebook_id !== currentNotebookId && (
                <Badge variant="outline" className={cn("text-xs", getNotebookColorClass(source.origin_notebook_name))}>
